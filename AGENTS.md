...
still apply, then commit and tag `vX.Y.Z` as `bot <bot@owngoal.dev>` and
dispatch `Release` on that tag (`gh workflow run release.yml --ref vX.Y.Z`):
...
owngoal-packages fetches the release at 04:00 UTC.
